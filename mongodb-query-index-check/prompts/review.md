...
- **🟠 high** — `Act2Runs.updateMany({ userId, status: 'RUNNING' }, { $set: { … } })` on a sharded collection. Replace with `bulkWrite` (one op per `_id`/`userId` pair, `ordered: false`) so each op carries its full shard key.

...
11. **`$lookup` joined via `$expr` instead of `localField` / `foreignField`.** Current MongoDB versions CAN use an equality index for `$expr: { $eq: ['$x', '$$x'] }` inside a `$lookup` sub-pipeline — an engineer disproved the old "forces a collscan" claim with an explain plan, so do not claim one. The `localField` / `foreignField` form is still preferred for plain equality joins (clearer, and index use doesn't depend on planner version or expression shape), so suggest the rewrite as a robustness/readability improvement at 🟡 medium. Flag at 🟠 high only when the `$expr` is more than a plain equality on the foreign field (computed expressions, `$in`, comparisons wrapped in `$cond`, …), where index use genuinely breaks. If the author replies with explain-plan evidence that the index is used, the finding is settled — don't re-post it.
...
13. **`projection` set without narrowing the return type.** The Mongo driver's `findOne` / `find` return the full document type even when a literal `projection` is passed — un-projected fields type-check as defined but are `undefined` at runtime. The repo convention is `findOne<ProjectedShape>(filter, { projection: PROJECTION as const })` where `ProjectedShape = PickByDotNotation<Doc, keyof typeof PROJECTION>`. Flag added reads that pass a literal `projection` but don't supply a generic type argument on the call. Exception: methods whose driver signature takes no result generic (e.g. `findOneAndDelete`) — suggest restructuring (typed `findOne` + separate delete) only when the untyped result is actually consumed. 🟡 medium.

## One-off migrations

Migration scripts (files under a `migrations/` directory, typically named with a timestamp prefix) run once and are deleted from the repo after execution. Engineers accept full scans there routinely, and review data shows they are right to: a one-off scan is often cheaper and safer than mutating the index surface of a busy collection. Findings on migration files follow special rules:
...
- **Severity for migration scans is about production impact while the migration runs**, not query elegance: an unbatched collection scan (or unbatched `updateMany`) over a collection that plausibly holds millions of documents (`Users`, `Act2Runs`, `Acts2`, `Webhooks`, request queues / storages) is still 🔴 critical — say what the blast radius is (working-set eviction, lock/CPU pressure). A scan that is batched, replica-read, or bounded to a small matched set is 🟢 low; state that the residual advice is operational (timing window, monitoring) so the author can accept it explicitly.
- **A reasoned author reply accepting the scan settles the finding.** "One-off, bounded set, runs off-peak" is a legitimate resolution. Do not re-post it on later runs — including when the migration file is renamed (timestamp prefixes change on rebase; match migration findings by content, not path).
...
- **🟡 medium** — `$lookup: { from: 'actorBadges', let: { actorId: '$_id' }, pipeline: [{ $match: { $expr: { $eq: ['$actorId', '$$actorId'] } } }] }`. A plain equality join written in the `$expr` form: current planners can still use the `{ actorId: 1 }` index, but the top-level `{ from: 'actorBadges', localField: '_id', foreignField: 'actorId', as: '…' }` form is more robust and clearer — suggest it, don't claim a collscan.
...
- 🔴 **critical** — Query touches a collection that exists, but **no index** covers any of its filter/sort fields → MongoDB will do a full collection scan. This is almost certainly a production-impacting issue. **A collection scan over a collection that plausibly holds millions of documents (`Users`, `Act2Runs`, `Acts2`, `Webhooks`, request queues / storages) is critical even in a daemon, maintenance path, or migration** — don't downgrade it because the code path is infrequent; the one exception is a migration scan that is already batched/replica-read/bounded (see "One-off migrations").
- 🟠 **high** — An index exists for that collection but **doesn't match the query**:
  ...
  - `$lookup` uses an `$expr` pipeline whose join condition is more than a plain equality (plain-equality `$expr` is 🟡 medium — see pattern 11).
...
- **Verify recommended index fields exist.** Before recommending a new or extended index, check every field against the collection's document type / schema usage in the repo. A past review recommended a four-field index where none of the fields existed on the type — all-null index keys.
- **Be conservative with `partialFilterExpression` in recommendations.** The planner only selects a partial index when the query filter provably subsumes the expression; a past adopted suggestion had to drop its proposed `removedAt: null` clause before Mongo would use the index in daemon queries. Propose a partial clause only when the query filters on those exact keys with equality-matchable values, and prefer mirroring a partial expression the collection's existing indexes already use.
- **Don't claim a collection scan without ruling out partial indexes.** Reconstruct which index the planner would pick, partial indexes included. If one covers the filter and the real problem is a blocking in-memory sort, say that instead, estimate the sorted set's size, and offer a smaller bounded batch as a first-class fix alongside index changes.
...
- **Check API contracts before suggesting keyset/cursor pagination.** A public API bound to offset/limit pagination can't switch; suggest a `hint`, bounded batches, or `readConcern` instead.
- **Negation → equality rewrites change semantics for missing fields.** When suggesting `{ $ne: true }` → `false` (or similar), say explicitly that documents missing the field behave differently and ask the author to verify field presence (a one-line count query) rather than asserting equivalence.
- **Make the suggested action complete and atomic.** Name every change required for the index to become usable and say which parts are insufficient alone — e.g. for an unanchored `$regex`, adding the `^` anchor is the load-bearing change; dropping the `i` flag alone still can't seek the index (a past finding was "fixed" by only dropping the flag).
- No findings → write `none` to `$RESULT_PATH` and exit silently.

### 3. Deduplicate against previous runs — and honor settled threads

A previous run on this PR may have already posted some of these findings, and humans may have replied. Historical review data shows duplicates are the check's most common failure mode (the same finding posted three times in five minutes; re-posts after a migration file rename; a re-post with escalated severity), so this step is as important as the analysis itself.
...
4. **A reasoned human reply settles a finding.** If any thread's replies contain a substantive engineering justification — an explain plan, a field-presence check, "one-off migration, bounded set", a correctness constraint, an API contract — treat that finding as settled: don't re-post it, and don't count it toward `$RESULT_PATH` severity on this or later runs (the human decision stands; a check that stays red after a reasoned decline just trains people to ignore it). Replies also often contain data (production timings, explain output) — fold it into your analysis of the remaining findings.
5. If no new findings remain, compute the max severity from the findings that are duplicates of **unsettled** threads (fixed-in-code findings must be re-verified against the new head, not assumed fixed), write it to `$RESULT_PATH`, and exit silently. Do not submit an empty review. Re-running the workflow must never silently green a check whose findings are still unaddressed and unsettled.

...
- **Severity threshold** that will fail the check: `$SEVERITY_THRESHOLD` (one of `low`, `medium`, `high`, `critical`).
- **Request-changes mode**: `$REQUEST_CHANGES_MODE`. When `true`, the workflow will fail when findings reach the threshold; when `false`, comments only.
...
For every changed or newly-added line inside `$CHANGED_FILES_PATH`, hunt for MongoDB collection method calls:
...
- For each file, fetch its diff. Prefer the GitHub MCP server (e.g. `mcp__github__pull_request_read` with `get_diff` / `get_files`, passing owner+repo from `$REPO` and `pull_number: $PR_NUMBER`). If that tool isn't available, fall back to `gh pr diff $PR_NUMBER --repo $REPO -- <path>` via Bash. Restrict your analysis to lines **added or modified** in the after-state of the diff.
...
- Compute the **maximum severity** across your findings.
- If `$REQUEST_CHANGES_MODE` is `true` and the max severity rank meets or exceeds `$SEVERITY_THRESHOLD`, submit with `event: REQUEST_CHANGES`.
- Otherwise submit with `event: COMMENT`.
- Severity ranks: `low=1`, `medium=2`, `high=3`, `critical=4`.

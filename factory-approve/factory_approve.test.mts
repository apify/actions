...
/** @param {Array<{ id: string, pass: boolean }>} results */
const failedIds = (results) => results.filter((result) => !result.pass).map((result) => result.id);
...
describe('runStaticChecks', () => {
    ...
    it('rejects when a reviewer requests changes, unless a later review resolves it', async () => {
        const reviews = [{ user: { login: 'reviewer' }, state: 'CHANGES_REQUESTED' }];
        const blocked = await runStaticChecks(makeContext({ reviews }));
        expect(failedIds(blocked)).toContain('no-changes-requested');

        const resolved = await runStaticChecks(
            makeContext({ reviews: [...reviews, { user: { login: 'reviewer' }, state: 'APPROVED' }] }),
        );
        expect(failedIds(resolved)).not.toContain('no-changes-requested');
    });
...
describe('matchingGlob', () => {
    ...
    it('returns the first matching pattern from a list', () => {
        ...
        expect(matchingGlob('scripts/foo.js', policy.denyGlobs)).toBe('scripts/**');
        expect(matchingGlob('src/packages/finances-server/src/x.ts', policy.denyGlobs)).toBe('**/finances-server/**');
...
describe('runClaudeCliVerdict', () => {
    ...
    it('parses the verdict file the CLI writes and passes the right restrictions', async () => {
        ...
        /** @type {any[]} */
        let seenArgs = [];
        const runProcess = /** @type {any} */ (
            (/** @type {any} */ _cmd, /** @type {any} */ args) => {
                seenArgs = args;
    ...
    it('fails closed when the CLI writes nothing, writes garbage, or does not run', async () => {
        ...
        const failed = await runClaudeCliVerdict({
            ...
            verdictDir: dir,
            policy,
            runProcess: /** @type {any} */ (() => ({ error: new Error('ENOENT') })),
        });
...
describe('writeHeadFiles', () => {
    it('writes nested post-change files and omits traversal, missing, and oversized ones', async () => {
        ...
        const contents = /** @type {Record<string, string | null>} */ ({
